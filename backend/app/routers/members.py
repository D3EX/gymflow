...
from ..models.models import User, Member, Attendance, Subscription, Plan
...
def get_members(db: Session = Depends(get_db), admin=Depends(require_admin)):
    return db.query(Member).join(User).all()
...
def get_my_profile(
    ...
    print(f"📈 This month check-ins: {checkins_this_month}")
    ...
    # ============================================================
    # FIXED: Get active subscription - more lenient check
    # ============================================================
    ...
    # If no active subscription, check if there's any subscription at all
    if not active_sub:
        active_sub = db.query(Subscription).filter(
            Subscription.member_id == member.id
        ).order_by(Subscription.end_date.desc()).first()
    ...
    if active_sub:
        ...
        print(f"📅 Membership: {plan.name if plan else 'None'}, Days left: {days_left}, Active: {is_active}")
    else:
        print("❌ No subscription found for member")
    ...
    print(f"📤 Response: total_checkins={total_checkins}, streak={streak}, membership={membership_info is not None}")
    return response
...
def get_member(member_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
...
def create_member(data: MemberCreate, db: Session = Depends(get_db), admin=Depends(require_admin)):
...
def update_member(member_id: int, data: MemberUpdate, db: Session = Depends(get_db), admin=Depends(require_admin)):
    member = db.query(Member).filter(Member.id == member_id).first()
    if not member:
        raise HTTPException(status_code=404, detail="Member not found")
...
def delete_member(member_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
    member = db.query(Member).filter(Member.id == member_id).first()
    if not member:
        raise HTTPException(status_code=404, detail="Member not found")
    user = member.user
